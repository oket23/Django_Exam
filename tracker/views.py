...
class DashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        food_today = FoodLog.objects.filter(user=user, date=target_date).aggregate(
            total_cal=Sum('calories'), total_p=Sum('protein'), total_f=Sum('fats'), total_c=Sum('carbs')
        )
        ...
        water_today = WaterLog.objects.filter(user=user, date=target_date).aggregate(total_water=Sum('amount_ml'))
        consumed_water = water_today['total_water'] or 0

        activity_today = ActivityLog.objects.filter(user=user, date=target_date).aggregate(
            total_burn=Sum('calories_burned'), total_duration=Sum('duration_minutes')
        )
        burned_calories = activity_today['total_burn'] or 0
        duration_minutes = activity_today['total_duration'] or 0

        food_logs_today = FoodLog.objects.filter(user=user, date=target_date)
        water_logs_today = WaterLog.objects.filter(user=user, date=target_date)
        activity_logs_today = ActivityLog.objects.filter(user=user, date=target_date)
        ...
        adjusted_carbs = profile.daily_carbs_goal + earned_carbs
        earned_water = int((duration_minutes / 60.0) * 500)
        adjusted_water = profile.daily_water_goal + earned_water

        return Response({
            "target_date": target_date.strftime('%Y-%m-%d'),
            "profile": profile_data,
            "charts": {"weight_history": weight_data},
            "today_summary": {
                "consumed_calories": consumed_calories, "consumed_protein": consumed_protein,
                "consumed_fat": consumed_fat, "consumed_carbs": consumed_carbs,
                "burned_calories": burned_calories, "active_minutes": duration_minutes,
                "net_calories": consumed_calories - burned_calories, "consumed_water": consumed_water
            },
            "dynamic_goals_today": {
                "target_calories": adjusted_calories, "target_protein": profile.daily_protein_goal,
                "target_fat": profile.daily_fat_goal, "target_carbs": adjusted_carbs,
                "target_water": adjusted_water, "earned_extra_carbs": earned_carbs,
                "earned_extra_water": earned_water
            },
            "logs_today": {
                "food": food_data_today, "water": water_data_today, "activity": activity_data_today
            }
        })
